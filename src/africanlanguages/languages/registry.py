...
from ..core.exceptions import DataLoadError, LanguageNotFoundError
...
class LanguageRegistry(metaclass=Singleton):
    ...
    def _load_data(self, data_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            raise DataLoadError(f"Invalid JSON in language data file: {e}")
        except Exception as e:
            raise DataLoadError(f"Failed to load language data: {e}")

    def _create_language_from_json(self, item: dict) -> Optional[Language]:
        """Create Language object from JSON item"""
        try:
            from .models import LanguageCodes, GeographicInfo
